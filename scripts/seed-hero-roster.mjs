...
//
// LOCAL ONLY — refuses to run anywhere else. It creates users and health data.
//
//   scripts/rls-local-setup.sh          # or a local stack with the prod schema
//   node scripts/seed-hero-roster.mjs
...
if (!/^https?:\/\/(127\.0\.0\.1|localhost)(:|\/|$)/.test(url)) {
  console.error(`REFUSING TO RUN against ${url} — this seeds users and health data. Local only.`)
  process.exit(1)
}
if (!serviceKey) {
...
const SOLO = {
  ...
  targets: { calories: 2100, protein: 150, carbs: 200, fat: 70, cardio_minutes: 30, steps: 8500, weight_goal: 66, weight_goal_unit: 'kg' },
