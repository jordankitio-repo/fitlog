...
try {
  ...
  // Generate a real Meeting prep brief (validates the new prompt + signals end-to-end)
  await pC.getByText('Meeting prep', { exact: true }).click()
  await pC.getByText('Meeting brief —', { exact: false }).waitFor({ timeout: 45000 }).catch(() => console.log('brief did not render in time'))
  await pC.waitForTimeout(800)
  const briefText = await pC.locator('pre').first().innerText().catch(() => '(no pre found)')
  console.log('--- MEETING BRIEF ---\n' + briefText.slice(0, 700) + '\n--- end ---')
