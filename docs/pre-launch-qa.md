...
<!-- Follow-up 2026-07-09: PWA install config pre-verified against prod (manifest/icons/no-flash/SW all correct); desktop Safari(WebKit)+Firefox smoke auto-verified clean. On-device install = still owner-manual (visual). -->
- [~] Install as PWA (mobile + desktop); icon/splash correct; theme applied before paint (no flash). _(**Config pre-verified 2026-07-09 (prod):** `manifest.webmanifest` = name/short_name Gardnr, `display:standalone`, scope `/`, `background_color #0a0a0a`, `theme_color #062120`, icons **192 + 512 + 512-maskable** all serve 200; `apple-touch-icon.png` + `favicon.svg/.ico/-32` all serve; the **no-flash** inline script (reads `gardnr-theme` + `prefers-color-scheme` → sets `data-theme` before paint) is present + CSP-hashed; `sw.js` serves. **NOTE:** `apple-mobile-web-app-capable` meta is absent — modern iOS honors the manifest `display:standalone`, so it should still open chrome-less; **if the iOS install opens WITH a Safari address bar, add that meta.** The actual on-device install/icon/splash render = **owner-manual visual check**.)_
...
- [x] **Pinch-zoom works** (a11y fix) and inputs don't auto-zoom on iOS focus. _(VERIFIED 2026-07-09 on real iOS Safari by the owner. **Two-stage fix:** the first attempt (`2219e08`, lock viewport on `focusin`) still zoomed on-device — iOS decides to zoom the instant focus lands, so `focusin` is too late. **Fixed for real in `960438c`:** move the `maximum-scale=1` lock to `touchstart`/`pointerdown` (tap-DOWN, before focus) — owner confirmed no zoom on the login field. Resting viewport keeps no `maximum-scale`, so pinch-zoom + axe stay clean (lock is transient during text entry). 16px inputs alone were insufficient on this device.)_
- [~] **Browser matrix:** Chrome, Safari, Firefox (desktop) + iOS Safari + Android Chrome — core loop works on each. _(Chromium exercised extensively via harness. **Desktop Safari (WebKit) + Firefox auto-verified 2026-07-09** via Playwright vs prod: landing + `/login` both render, Inter font renders (WebKit `fonts.check` true; Firefox visually confirmed Inter in the screenshot — `fonts.check` false was a timing quirk), **0 console errors, 0 CSP blocks, 0 failed requests**, login form present + interactive on both. The full authenticated signup→log loop is API-driven (low engine-specific risk) and was exercised on Chromium. **iOS Safari + Android Chrome = owner-manual.**)_
...
- [x] Check the error monitor (once Sentry is wired) shows no new exceptions from your run. _(RESOLVED 2026-07-09: `VITE_SENTRY_DSN` set in Vercel Production + redeployed → Sentry now LIVE. Verified: a thrown error on www.gardnr.fit → `POST …ingest.us.sentry.io/… 200`. Was dormant before (DSN unset). Project: gardnr-frontend / digigarden-llc. Sentry "Prevent Storing of IP Addresses" enabled (no geo). **FOLLOW-UP — source maps (WIRED 2026-07-09):** `@sentry/vite-plugin` added to `vite.config.js` (org `digigarden-llc` / project `gardnr-frontend`), emitting **hidden** maps that upload via **debug IDs** then delete from `dist` (`filesToDeleteAfterUpload`) so none are ever served; workbox SW maps disabled too. Guarded on `SENTRY_AUTH_TOKEN` — absent (local builds) → plugin skipped, no maps at all; a bad token logs a warning but never fails the build (verified). **LIVE 2026-07-09:** `SENTRY_AUTH_TOKEN` (org auth token) set in Vercel ("Production and Preview"); the `8fa715b` build log shows *"Successfully uploaded source maps to Sentry"* (debug-ID artifact bundle, release-tagged). Verified on prod: served JS has no `sourceMappingURL`, and `…​.js.map` → **HTTP 403** (uploaded to Sentry, not served on the CDN). Remaining optional confirmation: throw an error on www.gardnr.fit and check the Sentry issue's frames read `src/…:line`, not `index-<hash>.js:1`.)_
