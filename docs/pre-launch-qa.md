...
- [x] **Pinch-zoom works** (a11y fix) and inputs don't auto-zoom on iOS focus. _(VERIFIED 2026-07-09 on real iOS Safari by the owner (fully closed Safari/PWA first): the input-focus auto-zoom fix (`utils/iosInputZoom.js`) holds — no stuck zoom after tapping Sign in from a focused field, snaps back to 100%; pinch-zoom still works.)_
