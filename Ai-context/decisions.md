...
### Onboarding target calculator uses standard intake math — distinct from logged-data estimates (Jun 17)
**Reason:** The "no fabricated-confidence" rule is about asserting precision we *can't earn from our data* (logged-data TDEE needs weeks; per-client correlations are noise). It does NOT forbid the **standard by-hand intake math every coach already does** to set a *starting* target: Mifflin–St Jeor (or Katch–McArdle off a body-fat % the user supplies) → TDEE → a deficit/surplus from a chosen rate. That's a transparent calculation on **self-reported stats**, framed as a starting point the coach refines — not a claim mined from logged behavior.
**Consequences:** `utils/targetEstimate.js` is allowed and shipped (the calculator), and is **kept separate** from `energyBalanceRead` (which *is* the logged-data estimate and stays cautious per the rule above). Honesty guardrails on the calculator: deficit comes from a **target rate** (not a flat %); protein is on **goal/lean weight** (avoids over-prescribing for higher-bodyfat clients); the weeks-to-goal is labelled **linear/optimistic** ("at this rate; slows as you get lighter") rather than presented as precise. If we ever add a logged-data "your real maintenance is X" readout, it goes through `energyBalanceRead`'s provenance-visible treatment, not the calculator.

### Energy-balance read window is coach-selectable (2/3/4/6 wk, default 3 wk), on a fixed equal-weight window (Jul 18)
**Reason:** "Why 21 days?" — validated, not arbitrary. The read's method (maintenance = avg intake − weight-slope × 3500 cal/lb, 7-day rolling avg) is the industry standard: MacroFactor reverse-calculates TDEE from logged intake + scale weight and recalculates **weekly**, and manual physique coaches take a **weekly weight average and adjust on a 2–4 week cadence**. The window is bounded from *below* by noise, quantitatively: daily bodyweight swings **2–4 lb** (water/glycogen — each g glycogen holds 3–4 g water), which is *larger* than one week of real change (~1%/wk on 80 kg ≈ 1.7 lb/wk). So a trend is unreadable under ~2 weeks — you need ≥2 weekly averages to compare — which is why there is **no 1-week option** and 14 days is a hard floor. 2/3/4 weeks sit squarely in the validated 2–4 week band (MacroFactor is 120–170% more accurate than a static formula by then); **6 weeks is a deliberate extension** past the standard for a confident "phase" read on a stable client (its risk: averaging across a mid-window diet change — mitigated because the coach opts into it and the band widens when it over-smooths). Exposing a *selector* is NOT what consumer apps do (MacroFactor hides its window behind an adaptive weighting), but it *is* how a coach thinks by hand (new client → 2 wk; stable → 4–6 wk), so it fits a coach instrument specifically. A **fixed equal-weight window** (not recency-weighting) is chosen on purpose: it keeps the fit interpretable — the coach knows exactly which days are in the line — per the "facts + visible provenance" instrument doctrine (Jun 11). The honesty is carried by the **band widening** on shorter/noisier windows (slope SE ~1/√n and 1/span), not hidden.
**Consequences:** `energyBalanceRead` is already window-agnostic; `WINDOW_OPTIONS = [14, 21, 28, 42]`, default 21. `MIN_SPAN_DAYS` is capped to `min(14, windowDays−1)` because a 14-day window can only span 13 days (else 2 wk is unsatisfiable). The dedicated pull fetches `WINDOW_DATA_DAYS = 90` (≥ 2× the longest window) so switching windows and the prior-window trajectory never re-fetch. The choice persists per coach (localStorage `gardnr-eb-window`) and the AI brief uses the same window so panel and report agree. Insufficient windows degrade honestly (readiness checklist naming the short input; trajectory row drops when the prior window is thin) — never a fabricated read. **Future upgrade path (only when a coach names the need):** recency-weighting *within* the window (exponential decay, à la MacroFactor V3) — more accurate and kills the mid-window-change bias, but less transparent; keep the interpretable fixed fit until the accuracy gap is actually felt. Do NOT add windows > 6 wk without bumping `WINDOW_DATA_DAYS` (8 wk needs 112 days for its trajectory).

### Body-measurement cadence is phase-derived, not a new setting (Jul 19)
**Reason:** Measurements shipped with no cadence at all — purely ad-hoc, client-entered, no reminder — so a client could stop measuring and the trend small-multiples silently freeze without the coach noticing. Industry standard is **every 2–4 weeks, phase-dependent** (active fat loss ~2 wk, gain/maintenance ~4 wk); more often is mostly noise (circumference moves slowly and tape error is ~1–1.5 cm) and discouraging. The phase is already knowable from the client's weight goal (the same `direction` behind the weight target line), so the recommended cadence can be **derived, not configured** — matching the standard with zero setup, same move as the phase-aware energy-balance defaults. Making it a coach setting up front would be modularity nobody asked for; the honest gap was *surfacing staleness*, not letting coaches pick a schedule.
**Consequences:** `utils/measurementCadence.js`: `measurementCadenceDays(direction)` → 14 (cutting) / 28 (else); `measurementStatus` compares the latest measurement to that cadence. **Coach (ClientView)** gets the phase-precise read — an amber "Re-measure due · Nd" tinted pill (status-pill pattern, `--color-warning-dim`, not a solid badge — white-on-amber fails contrast) **[⚠️ superseded Sep 15 2026 — the status-pill pattern is gone; this cell is now plain amber text, see "Status is plain coloured text" below. The cadence logic and the coach/client asymmetry described here still stand.]** in the section header + relative age in the subtitle. **Client** gets a gentler reminder on the **steadier ~monthly (28-day) cadence regardless of phase, so it never nags**, styled as a green "action-to-do" tint (`--color-primary-dim` + green clock glyph, dismiss ✕, no colored left-edge — mirrors the existing Dashboard nudge-notice card exactly). It's surfaced on **both the Dashboard (their home) and the Log page**, because `HomeLanding`/`resolveClientLanding` smart-routes a client to *either* depending on whether they've logged — one surface alone would be missed. Dashboard dismissal is keyed by the last-measured date (re-shows only once they're overdue against a newer measurement). This asymmetry is deliberate: the coach's view is analytical/precise, the client's is an encouraging prompt. **Deferred (add only when a coach asks):** a coach-**overridable** cadence (a real setting, parallel to `checkin_interval_weeks`) and a measurement **nudge** (reuse the nudge edge fn with a `measure` reason). Female-client caveat noted for later: cycle water retention argues for monthly, cycle-aligned comparisons.
...
**Update (Jul 16) — the founder note is GONE; transparency now carries trust.** The "honest substitute for social proof is a founder's note" line above is superseded. The note was removed at the user's call: at any position it read as a stranger asking to be trusted *before* the product has shown anything, and it was the page's only vouch precisely because there are no users yet. What carries trust now is **radical transparency instead of a personality**: the **FAQ** answers the real objections plainly (what it costs, do my clients pay, what happens to their health data), and the **"proof plan"** invites a coach to test one client for one week and judge for themselves. Consistent with the no-fabrication doctrine — we substitute a *checkable claim* for *borrowed credibility*, not one kind of persuasion for another. (The hero is also no longer a "stylized mock" — it's four real screenshots as of Jul 12–13; see "The hero shows REAL screenshots" below.)
...
**⚠️ Partly superseded (Sep 15 2026).** Two claims here are no longer true: the ESLint rule is no longer *scoped per-file* (it covers all of `src/`) and no longer matches only a raw hex used as a `color:` value (it matches a hex bound to **any** property or const). The deferred "opinionated polish" was also done, on the coach roster, and produced the weight and spacing ramps. See the five Sep 15 entries under UI / UX, and `docs/design-system.md`.
...
### ~~Founding coach rate locked forever at $19/mo~~ — ⛔ SUPERSEDED Jul 13, 2026. DO NOT ACT ON THIS.
**Reason (original):** Reward and retain the first coaches who validate the product; create urgency for early adoption.
**Consequences (original):** Two live price IDs maintained (founding $19, standard $29). Founding price stays honored even after standard launches.

**Why it's dead:** Locking anyone in forever at $19 is a decision you cannot take back, and the number is almost certainly wrong. Market check (Jul 2026): competitors do **not** charge per seat — they bill a flat fee inside a client-count band. **TrueCoach** $29.98 (5 clients) / $69.98 (20) / $164.98 (50). **Practice Better** $35–155. **Healthie** $19 — which **caps at 10 clients** — and $129 for unlimited. So **$19 for an unlimited roster is roughly what Healthie charges for a 10-client plan: likely 3–5× underpriced.** It also means "no per-seat fee" differentiates against nobody; **the real wedge is NO CAPS.** And flat-forever caps net revenue retention at 100% by construction, while the Anthropic and Resend costs scale with every client on the roster.

**Replaced by:** *Pricing is deferred until ~10 coaches are weekly-active* (below).

### Pricing is DEFERRED until ~10 coaches are weekly-active
**Reason:** Every tier boundary anyone has proposed — 10 / 30 / unlimited — was **invented**. With zero users there is no data on what a real roster looks like, so building enforcement, proration and downgrade-scheduling around made-up numbers means building something you will immediately rebuild. The data that settles it (`coach_clients` roster size per coach) is already collected; it costs nothing to wait for it.
**Consequences:** `BILLING_ENABLED = false` stays. **No price appears on the landing page and none is promised to anyone** — the page says only "free while we're in early access". Do not lock founding coaches at a number. When it is time: shape it as flat-inside-a-band (the market norm), price well above $19, make the trial **14 days not 30** (a weekly-cadence tool proves itself in 2–3 check-in cycles), and grandfather the early coaches **privately by email** — a surprise "you're grandfathered" mail lands far harder than a promise on a page nobody has read. **Precondition already shipped:** the Stripe price is resolved server-side from the verified role (`8079e1f`) — a tier whose price the *client* picks is not a tier, it's a suggestion.

### Coach pricing = 3 billing cadences ($49/mo · $264/6mo · $490/yr); plumbing built, going-live still gated (Aug 2 2026)
**Reason:** The DEFERRED decision (above) settled *that* we wait to charge and *how* to shape it — flat, no caps, well above $19, 14-day trial. This picks the specific numbers and builds the mechanism so flipping billing on later is a config + flag change, not a build. The ladder is a **commitment ladder**: monthly ($49) is the anchor and the deliberately-worst per-month rate; the discount must *climb* with commitment (0% → ~10% → ~17%) so annual always beats 6-month, else nobody upgrades. Annual **$490 = "2 months free"** ($49×10) is the recognizable SaaS framing; **6-month $264 (~10% off)** is the trust-appropriate middle for a young, unproven product where a full year is a big ask (drop it once there's a retention track record). These are **starting numbers to move UP from** — the ~10-coach trigger still governs *when* we charge, and what the first coaches actually pay is the real validation. Trial is **14 days not 30**: a weekly-cadence tool proves out in 2–3 check-in cycles.
**Consequences:** `create-checkout-session` resolves the coach price from **role + a validated cadence key** (`STRIPE_COACH_PRICE_MONTHLY` / `_6MO` / `_ANNUAL` in server env), replacing the single `STRIPE_COACH_PRICE_ID`; the client sends only the cadence **label**, never a price (keeps the server-side-price precondition — a client-picked price is not a tier). Unknown/absent cadence → `monthly`, never a caller value. Coach `trial_period_days` 30 → **14**. Cadence is stamped into Stripe `subscription_data.metadata.cadence`; `stripe-webhook` persists it to a new **nullable `subscriptions.billing_interval`** (guarded write — a later metadata-less event can't wipe it), and `SubscriptionManager` shows the plan + renewal amount. **This is the `feat/coach-billing-cadences` branch; `BILLING_ENABLED` stays `false` — building the plumbing is deliberately separate from going live.** To launch (separate PR, gated by the trigger): create the 3 Stripe Prices under ONE Coach Product, set the 3 secrets, migrate `billing_interval` *before* deploying the webhook, flip the flag + landing copy in the **same** PR, and add any founding rate as a **SEPARATE locked Price**, not a deeper discount on this ladder. **Numbers live in three places that must agree:** `COACH_CADENCE_PRICE_IDS` keys (server) and the display strings in `CoachPaywall`'s `PLANS` array + `SubscriptionManager`'s `planAmount` map — change all three together.

### Beta coaches get ~6 months free with ZERO ongoing code — comp row or dashboard coupon (Aug 3 2026)
**Reason:** The first ~10 coaches are hand-picked and comped, and the founder's hard constraint is **not touching code to onboard each one**. Two facts shape the options: (1) the 14-day trial is added by `create-checkout-session` (server-side) while a coupon is entered later on Stripe's hosted page — they're independent, so a coupon can't "cancel" the trial; and (2) **Stripe coupon durations are whole units** (`once` / N whole months / `forever`), never days — so a "6-months-minus-14-days" coupon **cannot exist**, and `trial(14 d) + coupon(whole months)` can never sum to *exactly* 6 months. You can pick any two of {exact-6-months, auto-charge, no-code}, never all three.
**Consequences:** Chosen: **~6 months + 2 weeks, auto-charging, zero code** — the extra fortnight is a harmless beta bonus, and "clean 6 months" was never worth code. Per coach: they check out normally (14-day trial, card on file), then you add a **100%-off `duration:'once'` coupon to their subscription in the Stripe dashboard** (on the 6-month plan, `once` zeroes the single $264 invoice = 6 months free); it auto-charges $264 after, and you email them ~2 wk before so it isn't a surprise. **Zero-code, no-Stripe alternative** (if exactness matters more than auto-charge): insert a manual `subscriptions` row (`status='active'`, `current_period_end` ~6 mo out) — the access gate already honors `active` — and convert them by hand when it lapses. Do **not** build a beta flag / trial-suppression path unless you later want exact-6-months *and* auto-charge (the only combo that needs a one-time code change: skip the trial for flagged coaches, then a 6-month coupon).

### The landing page must not describe a product that doesn't exist
**Reason:** Three of the ten bugs found on Jul 12–13 were the marketing page lying about the product: it sold a $19 card-required trial while `BILLING_ENABLED = false` (nobody is charged, no card is taken), and its hero was a hand-drawn mock of a dashboard that did not exist. The copy had been **inferred from `create-checkout-session` without checking whether the paywall that calls it was reachable.** It wasn't.
**Consequences:** ALL landing copy lives in **`src/pages/landingContent.js`** — a content module with no markup. `Landing.jsx` renders it and **must not define copy of its own**. The module carries, at the top, a list of every load-bearing claim on the page **and how each was verified against the code**. If you change billing or privacy behaviour, change that file in the **same PR**. The test for landing copy is not "does it read well" — it's "**is it true, and how do I know**".

**Extended to feature lists (Jul 16).** When the footer got a **KEY FEATURES** chip band, every chip was audited against product code (edge functions, DB tables + RLS, computed utils, coach-facing UI) before shipping — and **"Re-engagement pauses" was dropped** because nothing implements it (the nearest real things are a 48h nudge cooldown + the client progress-lock, and it duplicated "Attention triage"). The rule is written into `landingContent.js` beside the list: **a feature chip points at the code that implements it, or it doesn't ship** — the same "is it true, and how do I know" test, applied to capability claims. Related honesty call the same session: the footer **social icons render only when `footer.social` is non-empty** — the row was scaffolded with an empty array and real accounts wired later (X/Instagram), so a dead `#` placeholder link never shipped to production.

### The hero shows REAL screenshots. Fake interactivity is worse than a static image.
**Reason:** The old hero was an interactive mock: a drawn dashboard with a "Review and send" button that did nothing. Fake interactivity is worse than a picture, because it invites a stranger to touch your product and then lies to them — and in a category thick with vaporware, that is the wrong first impression to buy. It was also mouse-only (`onMouseEnter` on bare `<div>`s), so it was dead on touch and failed **WCAG 2.2 SC 2.1.1 Keyboard (Level A)**.
**Consequences:** The hero is four **real** screens captured from the running app (`scripts/seed-hero-roster.mjs` → `scripts/shoot-hero.mjs`). Demo *data* is fine — "Maya Chen" needn't exist — but every pixel of *UI* must be real. Hotspot regions are **measured off the live DOM at capture time**, so callouts cannot drift from the picture. A click does nothing, deliberately: it's a photograph you can point at, not a puppet show. **The images go stale silently** — re-shoot after any change to the dashboard, heatmap, measurements card or energy-balance read.

### The hero is now the real product video; the demo below it is REAL interactivity (Jul 31 — evolves the static hero-tour)
**Reason:** Three product visuals ended up competing — the static hero-tour, a new ~1-min product video, and an interactive demo — with the roster screen in all three. A recorded video of the running app is a strictly stronger "show it" than static screenshots (real, in motion, narrated), so it takes the hero and the tour is retired. The interactive demo lower on the page is exactly what the old hero deliberately refused to be — but it is NOT the "fake interactivity" anti-pattern, because it **actually responds**: it sorts the roster, drafts a report, advances a guided spotlight tour. A click does what it claims; that was always the real rule, not "no interaction".
**Consequences:** Hero visual = `public/video/gardnr-product.mp4` (poster + `preload="none"`, nothing loads until play); `HeroTour` + `public/hero/*.webp` deleted. Each proof asset appears **once** — **watch (hero video) → tell (copy) → try (interactive demo)**. The demo's screens are **hand-built HTML matched to the real app**, not captured from it, so they carry the same staleness risk the screenshots did: the "no fake interactivity" rule still holds (the demo is functionally real), but a human must refresh `public/coach-demo.html` (and re-record the video) after any coach-side redesign.

### The interactive demo is a same-origin iframe; the prod CSP allows 'self' framing (Jul 31)
**Reason:** The demo reuses broad, generic class names (`.card`/`.btn`/`.row`) that would collide with the app's global CSS if injected inline, so it is isolated in an iframe (`public/coach-demo.html`). But the strict prod CSP — set deliberately for a consumer-health app, where clickjacking is a real threat — carried `frame-src 'none'` + `X-Frame-Options: DENY` + `frame-ancestors 'none'` and a **hashed-only** `script-src`. That blocked the iframe AND its inline script **in production**, while Vite dev (which applies none of those headers) showed it working. It shipped broken and was invisible until a fresh-browser prod check.
**Consequences:** In `vercel.json`, `frame-src`/`frame-ancestors` → `'self'` and `X-Frame-Options` → `SAMEORIGIN`: **same-origin framing (the demo) is allowed; cross-origin framing/clickjacking is still blocked** — the posture only loosens for our own origin. The demo's JS is **externalized to `/coach-demo.js`** so it loads under `script-src 'self'` (never re-add an inline-script hash for it — an external file is edit-proof). **Do not re-tighten `frame-src`/XFO to `none`/`DENY`** without also removing the embed. Standing rule this cost us: **the dev server does not enforce the prod CSP — verify any new embed or inline script against the production build (`vite preview`) with the prod CSP injected**, which is how this was finally caught.

### Analytics is same-origin, cookieless, and marketing-routes ONLY
**Reason:** Gardnr is a consumer health app bound by the **FTC Health Breach Notification Rule** and **WA My Health My Data**. Sending health-adjacent paths (`/log`, `/client/<uuid>`) to an analytics vendor is precisely the fact pattern behind the FTC's **GoodRx** and **BetterHelp** actions. Mounting the component inside `Landing` is **not sufficient**: `@vercel/analytics` `inject()` appends its script to `document.head` from an effect with no cleanup, so it **outlives the component** and keeps auto-tracking history changes straight into the authenticated app. `Privacy.jsx` said it didn't — and **a false statement in a privacy policy about health data is the violation itself.**
**Consequences:** Vercel Web Analytics (first-party, proxied through `/_vercel/insights`, so no third party and no CSP change). **Two guards, and both are load-bearing:** `route`/`path` props on `<Analytics>` (which set `data-disable-auto-track` on the script tag) and a **fail-closed `beforeSend` path allowlist**. No user IDs, no PII, no `beforeSend` enrichment. Never mount it in `App.jsx` or `index.html`.

### The founder note must be TRUE
**Reason:** The first draft was fiction. It opened *"I'm not a coach — I'm the client who kept letting his coach down"* and built to *"my coach couldn't notice"*. **He has never had a coach.** It would have shipped a fabricated origin story on the one section of the page whose entire job is to make him trustworthy — and a coach who found out would have been right to disbelieve everything else on the page too. It was caught only because he was asked, point blank, whether any of it was untrue.
**Consequences:** The note is the only thing on the page that vouches for Gardnr — there are no testimonials, because there are no users. What ships is what's true: he is not a coach, he is the person who *needs* one; training is easy and adherence is not; he researched what coaches have to work with and built for the gap. **The test is not "does it read well" — it's "would he say this out loud to a coach who then checked".**

### Coach outreach before more features
**Reason:** The landing page tells coaches their week is *"food screenshots buried in three message threads"* and *"rebuilding the same compliance spreadsheet."* **That came from desk research, not from a coach's mouth.** Everything else — the mobile fix, the bundle, the hero, the founder note — is downstream of whether that sentence is true, and nobody has checked. A page that names a pain nobody feels converts at zero no matter how fast it loads.
**Consequences:** `docs/coach-outreach.md` is the playbook: nine **past-tense** questions ("would you use…" is answered yes by everyone and means nothing), don't pitch, and ask for **one** client rather than a signup. The answer that should change the product: **if coaches say their problem is GETTING clients rather than SEEING compliance, this is an excellent tool for the wrong bottleneck** — and that is the most valuable thing that could be learned this year.
...
### The design system lives in `docs/design-system.md`; the skill is a loader (Sep 15 2026)
**Reason:** It was written into `.claude/skills/gardnr-design/SKILL.md` because a skill **auto-loads** whenever anyone touches UI, and a doc nobody opens governs nothing. That reasoning holds, but `.claude/` is a hidden agent directory — the document that rules how the product looks belongs on a path a person would think to open, beside `ai-resilience-design.md` and `passwordless-auth-design.md`. The obvious fix (write both) is the exact failure this session spent its time cleaning up: the system carried a rejected-patterns entry *prescribing* the tinted pill-with-a-dot that the same file banned nine rules later, because two copies existed and one was maintained.
**Consequences:** The full system moved to **`docs/design-system.md`** — one copy, conventional path, reviewable in a PR diff. `SKILL.md` keeps its frontmatter (so it still auto-fires on UI work) and is reduced to ~19 lines that say "read the doc" and **deliberately restate no rules**, so it has nothing that can fall out of date. Precedent: `frontend-design-audit` splits `SKILL.md` from `references/`. All cross-references repointed (`eslint.config.js`, `architecture.md`, `current-state.md`, `ui/Panel.jsx`, the `[[design-system]]` memory). **Rule going forward: a second copy of a rule is a bug, not redundancy** — link to the doc instead.
...
### A commit action is filled green; the exception is REPETITION, not conditionality (Sep 15 2026)
**Reason:** The ladder's rank-1 rule read *"max one accent fill per VIEW."* Applied literally to ClientView it demoted every commit on the page — Save targets, Add note, Mark reviewed, Send to client all went neutral — and a 14-panel record ended up with no affirmative action anywhere. The rule had been generalised from a single bad instance ("Send invite" filled green at the foot of the roster, making an occasional setup task the loudest thing on a scanning screen) into a per-screen quota. The tempting replacement, *"permanent is green, conditional is neutral,"* is also wrong: the "N check-ins to review" CTA is conditional (`checkInsToReview > 0`) and is green. What actually causes inflation is **repetition** — nine Nudges down a roster compete because they are the same button answering nine times, and standing colour there reads as *this row is marked* (state) rather than *do this* (action), which is B2's line.
**Consequences:** Rank 1 is **max one per PANEL, never repeated down a list**. `primary` for a panel's or a view's single commit (Save targets, Add note, Mark reviewed, Send to client, Send invite, the check-ins CTA); `action` — neutral at rest, green revealed on hover/press — for a commit that renders per row (Nudge). Fixed in `docs/design-system.md` D3 + the elevation ladder and in `Button.jsx`'s ladder comment. **General lesson: a rule derived from one instance usually encodes the instance, not the principle.**

### The filled accent is a separate token from the accent hue (Sep 15 2026)
**Reason:** `--color-primary` was doing two jobs with opposite requirements. In dark it must stay bright (`#22c55e`) to work as **text** — links, focus rings, active labels, status green on a near-black page. At that brightness, poured across a whole button as a **surface**, it glares. One token cannot serve both.
**Consequences:** `--color-primary-fill` / `--color-on-primary-fill` added. Dark = `#15803d` (green-700) with white type at **5.0:1**; light collapses back to `--color-primary` (`#166534`, white at **7.1:1**) because there the brand green is already dark. Swept all ten filled-accent surfaces: `Button` `primary` (and therefore `Pill`, so the View chips and the buttons stay matched *by construction*), chat FAB, PWA prompt, reset-password, Dashboard's check-in pill + three count chips, Log's unit pill. **`ui/Badge`'s `solid` tone deliberately still uses `--color-primary`/`--color-on-accent`** — it takes an arbitrary `color` (Dashboard passes `--color-error`) and its foreground must flip with the fill's lightness, so a blanket swap would have put white on light red at 2.5:1. Give it an `onColor` prop if solid badges ever spread.

### A blanket `button:hover` silently overrides every designed control (Sep 15 2026)
**Reason:** Two legacy rules in `index.css` applied to every `<button>`: `button:not(:disabled):hover { filter: brightness(1.08); transition: filter .15s }` and `.btn:hover { filter: brightness(1.12) }` / `.btn:active { transform: scale(0.97) }`. `Button.jsx` narrates the 1.12 rule *in the past tense* — only the JS half had ever been deleted. They (1) rendered every variant's hover **12% brighter than the token it was tuned to**, so the luminance tables in the written system described the spec rather than the screen; (2) ran `scale(0.97)` on top of the ladder's 0.5px sink, two press animations at once; and (3) **hijacked the transition list** — a `transition` declared inside a `:hover` rule replaces the component's own for as long as the cursor is over it. Measured on the top-bar Feedback item: at `hover+20ms` the background had already snapped with no easing while only the filter animated; at `leave+20ms` the filter had snapped back while only the background eased. The fill jumped in and the brightness jumped out. **None of this is visible in the component's CSS, because the component's CSS is correct.**
**Consequences:** The blanket stays as a fallback for a genuinely unstyled `<button>`, but anything declaring its own hover carries **`ds-control`** and is excluded: `Button`, `Pill`, `.gnav-util`, `.cv-rail-item`, `.cv-rail-back`, `.ds-disclosure`, `.ds-sortbtn`, `.ds-fixbtn`, `.gw-tile`. **Add it to anything new that styles its own `:hover`.** Related: **never transition from `transparent`** — it is `rgba(0,0,0,0)`, so a fade to an opaque colour interpolates through translucent black and arrives as a smear; start from the surrounding surface's own colour so both ends are opaque.

### The rail pages own their layout; the rail is a full-height sidebar (Sep 15 2026)
**Reason:** Supersedes the Jun 16 section-rail decision's *placement* (its scroll-spy + live-order contract still stands). The rail lived inside `<main>`'s centred `max-width`, so it could never reach the viewport edge — it read as a card floating mid-page with a band of empty background to its left. Vercel and Cloudflare both draw the sidebar as a contained surface flush to the window edge; that containment is what makes a short list read as deliberate rather than scattered.
**Consequences:** `/client/:id` and `/profile` are `ownsLayout` in `App.jsx` — `<main>` supplies no padding and `.cv-shell` / `.cv-main` supply their own (including the client chat-FAB bottom clearance `<main>` used to give). Rail is 260px at `x=0`, full height under the 56px nav, one right border. Content is left-aligned from the rail with a 1480px cap, **not re-centred** — re-centring reopens the gap the sidebar just closed.

### A sidebar row is not a control row; a scroll-driven marker must be type-only (Sep 15 2026)
**Reason:** Two separate mistakes from borrowing the references' visual language for a different mechanism. (1) Rail rows had inherited the roster's **30px control height** with 2px gaps and 13px labels — right for buttons above a dense table, far too tight for a standing nav column. (2) The active row was a filled block, but Vercel's and Cloudflare's sidebars mark which **route** you are on, so their fill is placed once and sits still; ours is an in-page TOC on scroll-spy, so a filled block slid down the column the whole time the page moved and the eye tracked it instead of the content.
**Consequences:** Rows are 40px / 6px gaps / 14px labels, which fills the column edge-to-edge at an 800px viewport. **A fill belongs to the POINTER (hover); a scroll-driven marker is colour only** — `--color-muted` → `--color-text` plus full-strength icon, no background and deliberately **no weight change**, since medium→semibold re-advances the glyphs and each row would reflow as the marker passed over it. The active rule declares **no `background` at all** (not even `transparent`), because it sits after `:hover` and would otherwise make the active row the one row that stops responding to the pointer.

### Top-bar items are not buttons (Sep 15 2026)
**Reason:** Feedback was `<Button variant="muted">`, identical to Nudge — so the chrome bar and the client record used the same control for "email the team a note" and "email this client a reminder." A top bar is a place you leave *from*, not a place you act *in*; a raised control there competes with the one real action on the page below.
**Consequences:** `.gnav-util` — label + glyph that darken on hover, sharing `.gnav-link`'s shape, on an 8px rounded rectangle rather than a capsule (a 999px pill is the app's shape for a chip you press repeatedly). The notification bell had **no hover of its own at all** and was living off the blanket above; it now carries the same class, so bell and Feedback are byte-identical in both themes by construction. Both glyphs are 20px / stroke 2.

### Measure the SOURCE as well as the render — a thin fixture can fake a clean screen (Sep 15 2026)
**Reason:** Complements the Sep 15 "audit the rendered page, not the source" decision, from the other direction. ClientView's *rendered* numbers barely moved across the whole pass (4716px → 4699, 33 bordered boxes → 32, depth already 1) because the seeded demo client exercises almost none of the deep paths — the three-deep Sent-reports tree needs sent reports, the bordered meal cards need a logged meal. Source metrics moved enormously over the same diff.
**Consequences:** The render is the truth about what a reader sees on **one dataset**; the source is the truth about what the page **can draw**. Report both. Related trap found the same way: a mechanical spacing sweep rewrote `padding: 10` inside `chartOptions` to `var(--space-10)` and took the page down with a `Maximum call stack size exceeded` out of chart.js's font resolver — **`npm run build` does not catch it**, esbuild is happy and the crash is runtime-only. Screenshot after any mechanical sweep; a green build is not evidence.
